...
namespace QuickEngine.Extensions
{
	public static class VectorExtensions
	{
		...
		{
			Vector2 normalized = @base.normalized;
			float d = Vector2.Dot(v2, normalized);
			return normalized * d;
		}
		...
		{
			return v2.Round(decimals).ToString();
		}
		...
		{
			Vector3 normalized = @base.normalized;
			float d = Vector3.Dot(v3, normalized);
			return normalized * d;
		}
		...
		{
			return new Vector3((float)Math.Round(v3.x, decimals), (float)Math.Round(v3.y, decimals));
		}

		public static string ToString(this Vector3 v3, int decimals = 0)
		{
			return v3.Round(decimals).ToString();
		}
